...
""" simulate keyboard, mouse, hardware inputs """
...
class _INPUTunion(ctypes.Union):
    """ the union for the inputs above """
...
class INPUT(ctypes.Structure):
    """ the wrapper for the inputs """
    _fields_ = (('type', DWORD),
                ('union', _INPUTunion))


# these names are all conform Windows, not Puython
...
ORDER = ascii_letters + digits + ' \b\r\t'
...
def _send_keypress(code: int):
    """ handle keypress """
    SendInput(Keyboard(code, flags=0), Keyboard(code, flags=2))


def volume_up():
    """ increase the master volume """
    _send_keypress(VK_VOLUME_UP)


def volume_down():
    """ decrease the master volume """
    _send_keypress(VK_VOLUME_DOWN)
